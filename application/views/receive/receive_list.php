      ...
      <section class='content'>
            ...
            <div class='col-12'>
              <div class='card'>
                <div class='card-header'>
                    <div class="row">
                    <div class="col-md-6">
                  <h3 class='card-title'>RECEIVE LIST <?php echo anchor('index.php/receive/create/','Create',array('class'=>'btn btn-primary btn-sm'));?></h3>
                    </div>
                    <div class="col-md-6">
        ...
        function data_range(first,last){
          ...
          var table = $('#mytable').DataTable( {
              ...
              searching: true,
              "ajax": {
                "url": "<?php echo base_url()?>index.php/receive/range/",
                "dataSrc": "",
                "data": function(data) {
                  data.first = first;
                  data.last = last;
                  ...
                  { "data": null,
                    render: function ( data, type, row ) {
                      if (data.status == "received") {
                        return data.status + " in " + data.nama_wr_penerima
                      }
          ...
          $('#mytable').on( 'click', '#update', function (e) {
            e.preventDefault();
              var data = table.row( $(this).parents('tr') ).data();
              if (data != null) {
                window.location = '<?php echo base_url()?>index.php/receive/update/'+data.id;
              }
          } );
          $('#mytable').on( 'click', '#print', function (e) {
            e.preventDefault();
              var data = table.row( $(this).parents('tr') ).data();
              if (data != null) {
                window.open('<?php echo base_url()?>index.php/receive/pdfReceive/'+data.id,'_blank');
              }
          } );
          $('#mytable').on( 'click', '#delete', function (e) {
            e.preventDefault();
              ...
              if (data != null) {
                    ...
                    swal({
                      title: "Confirmation",
                      text: "Are your sure want to delete Received Delivery No."+data.kode + "/" + data.bulan + "/" + data.tahun+ " ?",
                      icon: "warning",
                      buttons: true,
                      dangerMode: true,
                    })
                    .then((willDelete) => {
                      if (willDelete) {
                       ...
                       window.location= '<?php echo base_url()?>index.php/receive/deleteAll/'+data.kode;
                      } else {
