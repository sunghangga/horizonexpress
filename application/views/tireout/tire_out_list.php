      ...
      <section class='content'>
            ...
            <div class='col-12'>
              <div class='card'>
                <div class='card-header'>
                    <div class="row">
                    <div class="col-md-6">
                      <h3 class='card-title'>TIRE OUT LIST <?php echo anchor('index.php/tire_out/create/','Create',array('class'=>'btn btn-primary btn-sm'));?></h3>
                  </div>
                  <div class="col-md-6">
        ...
        <script>
          ...
          var table = $('#mytable').DataTable( {
              ...
              searching: true,
              "ajax": {
                "url": "<?php echo base_url()?>index.php/tire_out/range/",
                "dataSrc": "",
                "data": function(data) {
                  data.first = first;
                  data.last = last;
              ...
              "columnDefs": [
                  { targets: -2, "width": "90px", render: function(data){return moment(data.create_at).format('D MMM YYYY'); }},
                  { targets: -1, "width": "100px" },
                  { targets: [0,2], "width": "150px" },
                  { targets: [3,4,5,6], "width": "100px" },
              ]
          } );
          ...
          $('#mytable').on( 'click', '#info', function (e) {
            e.preventDefault();
              ...
              if (data != null) {
                window.location = '<?php echo base_url()?>index.php/tire_out/read/'+data.id;
              }
          } );
          ...
          $('#mytable').on( 'click', '#print', function (e) {
            e.preventDefault();
              var data = table.row( $(this).parents('tr') ).data();
              if (data != null) {
                window.open('<?php echo base_url()?>index.php/tire_out/pdfBarangKeluar/'+data.id,'_blank');
              }
          } );
          ...
          $('#mytable').on( 'click', '#update', function (e) {
            e.preventDefault();
              var data = table.row( $(this).parents('tr') ).data();
              if (data != null) {
                window.location = '<?php echo base_url()?>index.php/tire_out/update/'+data.id;
              }
          } );
          $('#mytable').on( 'click', '#delete', function (e) {
            e.preventDefault();
              ...
              if (data != null && result) {
                window.location = '<?php echo base_url()?>index.php/tire_out/delete/'+data.id;
              }
          } );
